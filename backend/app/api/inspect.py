...
async def inspect(
    ...
    # ?? content_type/????? file_type, ???? max_size
    # (??: ???? image/video ?? max_image_size, ??????? 20MB)
    ...
    if len(file_bytes) > max_size:
        raise HTTPException(
            ...
            detail={
                "code": "FILE_TOO_LARGE",
                "message": f"File exceeds max size for {file_type} ({max_size // (1024*1024)} MB)",
            },
        )
...
async def inspect_batch(
    ...
    for i, uf in enumerate(files):
        ...
        fmax = settings.max_image_size if ftype == "image" else (settings.max_video_size if ftype == "video" else settings.max_image_size)
        if len(fb) > fmax:
            raise HTTPException(
                status_code=413,
                detail={"code": "FILE_TOO_LARGE", "message": f"File #{i} ({ftype}) exceeds {fmax // (1024*1024)} MB"},
            )
...
@router.post("/{algorithm_code}/from-upload/{session_id}", response_model=InspectionCreateOut, status_code=202)
async def inspect_from_upload(
    algorithm_code: str,
    session_id: str,
    request: Request,
    meta: str = Form("{}"),
    ...
    upload_sess = r.scalar_one_or_none()
    if upload_sess is None:
        raise HTTPException(
            status_code=404,
            detail={"code": "UPLOAD_SESSION_NOT_FOUND", "message": f"Session {session_id} not found"},
        )
    if upload_sess.status != "completed" or upload_sess.offset != upload_sess.total_size:
        raise HTTPException(
            status_code=409,
            detail={"code": "UPLOAD_INCOMPLETE", "message": f"Upload incomplete: {upload_sess.offset}/{upload_sess.total_size}"},
        )

    settings = get_settings()
    tmp = Path(upload_sess.tmp_path)
    if not tmp.exists() or tmp.stat().st_size != upload_sess.total_size:
        raise HTTPException(
            status_code=410,
    ...
    try:
        ...
        logger.exception("MinIO upload failed (TUS finalize)")
        inspection.status = "FAILED"
        inspection.error_code = "STORAGE_ERROR"
        inspection.error_message = str(e)
        await session.commit()
    ...
    await audit.log(
        ...
        user_agent=request.headers.get("user-agent"),
        request_id=request.headers.get("x-request-id"),
        request_meta={"algorithm_code": algorithm_code, "file_size": len(file_bytes), "tus_session_id": session_id},
        result=AuditResult.SUCCESS,
    )
    await session.commit()
    await session.close()
